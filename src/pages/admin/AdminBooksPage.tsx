...
export function AdminBooksPage() {
  ...
  const fetchItems = async () => {
    try {
      ...
      const books: any[] = res.data?.data || res.data || [];
      setItems(books);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    api.get('/orders').then(res => {
      ...
      const rev = paidOrders.reduce((acc: number, o: any) => acc + Number(o.total || 0), 0);
      setRevenue(rev);
    }).catch(() => {});
  }, []);
  ...
  const handleEdit = (item: any) => {
    ...
    const images = Array.isArray(item.images) && item.images.length > 0
      ? item.images : (item.coverImage ? [item.coverImage] : []);
    setEditingItem({ ...item, images, coverImage: images[0] || item.coverImage || '' });
    setIsModalOpen(true);
  };
  ...
  const handleSave = async (data: any) => {
    if (modalMode === 'ADD') {
      ...
      await api.patch(`/books/${editingItem.id}`, data);
  ...
  const confirmDelete = async () => {
    if (itemToDelete) {
      await api.delete(`/books/${itemToDelete.id}`);
    ...
    <div>
      ...
      <div style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.85rem', margin: 0 }}>Manage your books and e-books.</p>
        </div>
      ...
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '1.25rem', padding: '1.75rem' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead>
              <tr>
                {['Cover', 'Title', 'Price', 'Status', 'Category', 'Featured', 'Orders', 'Actions'].map(h => (
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} style={{ padding: '2rem', textAlign: 'center', color: 'rgba(255,255,255,0.4)' }}>Loading books...</td></tr>
              ) : filteredItems.length === 0 ? (
                <tr><td colSpan={8} style={{ padding: '2rem', textAlign: 'center', color: 'rgba(255,255,255,0.25)', fontStyle: 'italic' }}>No books found</td></tr>
              ) : (
                filteredItems.map((item: any) => (
                  <tr key={item.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    <td style={{ padding: '1rem 0.75rem' }}>
                      {item.coverImage ? (
                        <div style={{ width: 30, height: 45, borderRadius: 2, overflow: 'hidden', background: '#000' }}>
                          <img src={resolveImageUrl(item.coverImage)} alt={item.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        </div>
                      ) : (
                        <div style={{ width: 30, height: 45, borderRadius: 2, background: 'rgba(255,255,255,0.08)' }} />
                      )}
                    </td>
                    <td style={{ padding: '1rem 0.75rem', color: 'rgba(255,255,255,0.85)' }}>
                      <strong>{item.title}</strong>
                      <div style={{ fontSize: '0.73rem', color: 'rgba(255,255,255,0.35)' }}>{item.author}</div>
                    </td>
      ...
      {orderBook && (
        <OrderListModal
          entityId={orderBook.id}
          entityTitle={orderBook.title}
          entityType="book"
